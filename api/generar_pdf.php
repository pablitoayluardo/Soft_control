...
// Evitar salida antes de headers
ob_start();
...
try {
    ...
    echo '<h2>Error generando el PDF</h2>';
    echo '<p><strong>Mensaje:</strong> ' . htmlspecialchars($e->getMessage()) . '</p>';
    echo '<p><strong>Archivo:</strong> ' . htmlspecialchars($e->getFile()) . '</p>';
    echo '<p><strong>Línea:</strong> ' . $e->getLine() . '</p>';
    echo '<p><strong>Clave de acceso:</strong> ' . htmlspecialchars($claveAcceso ?? 'No proporcionada') . '</p>';
    
    // Mostrar información adicional de debugging
    if (isset($factura)) {
        echo '<h3>Información de la factura encontrada:</h3>';
        echo '<pre>' . htmlspecialchars(print_r($factura, true)) . '</pre>';
    }
    
    if (isset($detalles)) {
        echo '<h3>Detalles encontrados:</h3>';
        echo '<pre>' . htmlspecialchars(print_r($detalles, true)) . '</pre>';
    }
